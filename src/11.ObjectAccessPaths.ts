type JoinString<T extends string, K extends string, Separator extends string> =
	T extends '' ?
	K extends '' ? never : K : (K extends '' ? T
		: `${T}${Separator}${K}`)
...
type ObjectAccessPaths<T extends Record<string, any>, Prev extends string = '', K = keyof T> =
	K extends keyof T
	? (K extends string ? ((T[K] extends Record<string, any> ? ObjectAccessPaths<T[K], JoinString<Prev, K, '.'>>
		: JoinString<Prev, K, '.'>)) : never)
	: never
